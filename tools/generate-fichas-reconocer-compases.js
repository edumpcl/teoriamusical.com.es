'use strict';
/**
 * Ficha imprimible de «reconocer compás» (A4, una cara) con su hoja de soluciones.
 *
 *   node tools/generate-fichas-reconocer-compases.js            -> ficha + soluciones + preview
 *   node tools/generate-fichas-reconocer-compases.js --png=DIR  -> además, captura de cada hoja
 *
 * Doce compases escritos SIN cifra (dibujarMedida con sinCifra:true): el alumno
 * escribe la cifra en la casilla. Cuatro simples, cuatro compuestos y cuatro
 * mezclados. Las semillas de cada lote se buscaron para que cada grupo incluya al
 * menos un compás AMBIGUO (3/4 y 6/8 duran igual y se escriben igual si no hay
 * corcheas que muestren la agrupación): la hoja de soluciones da las dos cifras
 * por buenas en esos casos, con la misma explicación que en pantalla.
 * Reutiliza assets/js/completar-compas-engine.js (generarMedida/dibujarMedida) y
 * assets/js/reconocer-compas-engine.js (generarLote/equivalentes/explicar).
 * La teoría se audita con tools/verificar-reconocer-compas.js.
 */
...
/* Los lotes de la ficha: grupo, cuántos, semilla (buscadas para que cada grupo
   incluya al menos un compás ambiguo). El verificador los reutiliza. */
const LOTES = [
  ['simples', 6, 9000],
  ['compuestos', 6, 9000],
  ['mezcla', 6, 9000],
...
const TIT = { simples: '1. Compases simples (2/4, 3/4, 4/4)', compuestos: '2. Compases compuestos (6/8, 9/8, 12/8)', mezcla: '3. Mezclados' };
  ...
  <div class="cab">
    <div class="cab-top">
      <div>
        ...
        <h1>Reconocer compás${solucion ? '<span class="sol-tag">SOLUCIONES</span>' : ''}</h1>
      </div>
      <img class="logo" src="${LOGO}" width="34" height="35" alt="">
    </div>
    <p class="instr">Cada compás está escrito sin su cifra. Escribe en la casilla el compás que le corresponde (numerador sobre denominador). Algunos se pueden escribir de más de una forma: en ese caso, cualquiera de las dos es correcta.</p>
    ${solucion ? '' : '<div class="datos"><span><b>Nombre:</b></span><span><b>Curso:</b></span><span><b>Fecha:</b></span></div>'}
  </div>
  <div id="cuerpo"></div>
  <div class="pie"><span>Compases &middot; reconocer compás${solucion ? ' &middot; soluciones' : ''} &middot; teoriamusical.com.es/ejercicios/compases/</span><span>teoriamusical.com.es</span></div>
</div></body></html>`;
}

/* Dentro de la página, con los motores cargados. */
function montar({ LOTES, solucion }) {
  const T1 = window.tmCompletarCompasData, T2 = window.tmReconocerCompasTest;
  const cuerpo = document.getElementById('cuerpo');
  let n = 0;
  LOTES.forEach(([grupo, cuantos, semilla]) => {
    const h = document.createElement('h2'); h.textContent = window.__tmTit[grupo]; cuerpo.appendChild(h);
    const rej = document.createElement('div'); rej.className = 'rejilla'; cuerpo.appendChild(rej);
    T2.generarLote({ grupo, n: cuantos }, semilla).forEach(it => {
      ...
      rej.appendChild(c);
      T1.dibujarMedida(c.querySelector('.svg'), it.compasBase, it.elems, { w: 400, sinCifra: true });
    });
  });
  return n;
}
