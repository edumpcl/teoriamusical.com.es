'use strict';
/**
 * Ficha imprimible de «grupos de valoración especial» (A4, una cara) con su
 * hoja de soluciones.
 *
 *   node tools/generate-fichas-grupos-valoracion-especial.js            -> ficha + soluciones + preview
 *   node tools/generate-fichas-grupos-valoracion-especial.js --png=DIR  -> además, captura de cada hoja
 *
 * Dieciocho compases con un tresillo, dosillo, cuatrillo o seisillo cada uno,
 * seis de cada nivel (fácil/medio/difícil). El alumno escribe en la casilla la
 * figura a la que equivale el grupo marcado. Reutiliza
 * assets/js/completar-compas-engine.js (tabla de figuras) y
 * assets/js/grupos-valoracion-especial-engine.js (generarLote/dibujarConGrupo/
 * explicar). La teoría se audita con tools/verificar-grupos-valoracion-especial.js.
 */
...
const TIT = { 1: '1. Fácil (ritmo uniforme)', 2: '2. Medio (con algún silencio)', 3: '3. Difícil (ritmo mixto)' };
  ...
  <div class="cab">
    <div class="cab-top">
      <div>
        ...
        <h1>Grupos de valoración especial${solucion ? '<span class="sol-tag">SOLUCIONES</span>' : ''}</h1>
      </div>
      <img class="logo" src="${LOGO}" width="34" height="35" alt="">
    </div>
    <p class="instr">Cada compás tiene un grupo de valoración especial (tresillo, dosillo, cuatrillo o seisillo). Escribe en la casilla la figura sencilla a la que equivale el grupo entero.</p>
    ${solucion ? '' : '<div class="datos"><span><b>Nombre:</b></span><span><b>Curso:</b></span><span><b>Fecha:</b></span></div>'}
  </div>
  <div id="cuerpo"></div>
  <div class="pie"><span>Compases &middot; grupos de valoración especial${solucion ? ' &middot; soluciones' : ''} &middot; teoriamusical.com.es/ejercicios/</span><span>teoriamusical.com.es</span></div>
</div></body></html>`;
}

/* Dentro de la página, con los motores cargados. */
function montar({ LOTES, solucion }) {
  const T1 = window.tmCompletarCompasData, T2 = window.tmGruposVETest;
  ...
  LOTES.forEach(([nivel, cuantos, semilla]) => {
    ...
    T2.generarLote({ nivel, n: cuantos }, semilla).forEach(it => {
