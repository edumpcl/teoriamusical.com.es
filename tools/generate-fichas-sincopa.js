'use strict';
/**
 * Ficha imprimible de «síncopa» (A4, una cara) con su hoja de soluciones.
 *
 *   node tools/generate-fichas-sincopa.js            -> ficha + soluciones + preview
 *   node tools/generate-fichas-sincopa.js --png=DIR  -> además, captura de cada hoja
 *
 * El ejercicio en pantalla genera fragmentos al vuelo (assets/js/sincopa-engine.js,
 * generarFragmento: sortea compás —2/4, 3/4 o 4/4— y luego uno de los moldes
 * válidos para ese compás), así que para que la ficha en PDF salga siempre
 * igual se usa un mulberry32 con semilla fija en vez de Math.random. Son los
 * mismos moldes que en pantalla, verificados contra /diccionario-musical/
 * compases/sincopa/ y contra tools/verificar-sincopa-dificil.js.
 * El alumno rodea con un círculo la nota donde cree que empieza la síncopa,
 * o marca la casilla «No hay síncopa». En soluciones, la nota (o el par
 * ligado) sale en rojo, y la casilla se marca en rojo cuando no hay síncopa.
 */
const path = require('path');
const fs = require('fs');

const ROOT = path.join(__dirname, '..');
/* sincopa-engine.js usa la API de VexFlow 4 (Vex.Flow, opciones en
   snake_case: first_note/last_note...), la misma que carga la página real
   por CDN. El paquete node_modules/vexflow es la v5 (API distinta, en
   camelCase) y rompe StaveTie con esas opciones, así que aquí se carga el
   mismo build por red en vez del paquete de Node. */
...
/* Semilla fija + cuántos fragmentos trae la ficha. El verificador reutiliza esto. */
const SEMILLA = 7000;
...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .celda.ancha svg { height: 58px !important; }
  ...
  <div class="cab">
    <div class="cab-top">
      <div>
        ...
        <h1>Síncopa${solucion ? '<span class="sol-tag">SOLUCIONES</span>' : ''}</h1>
      </div>
      <img class="logo" src="${LOGO}" width="34" height="35" alt="">
    </div>
    <p class="instr">Rodea con un círculo la nota donde crees que empieza la síncopa (la que ataca en tiempo o parte débil y se liga al tiempo fuerte siguiente). Si el fragmento no tiene ninguna síncopa, marca la casilla «No hay síncopa».</p>
    ${solucion ? '' : '<div class="datos"><span><b>Nombre:</b></span><span><b>Curso:</b></span><span><b>Fecha:</b></span></div>'}
  </div>
  <div id="cuerpo" class="rejilla"></div>
  <div class="pie"><span>Compases &middot; síncopa${solucion ? ' &middot; soluciones' : ''} &middot; teoriamusical.com.es/ejercicios/compases/sincopa/</span><span>teoriamusical.com.es</span></div>
</div></body></html>`;
}
...
function montar({ SEMILLA, N_FRAGMENTOS, solucion }) {
  ...
  for (let i = 0; i < N_FRAGMENTOS; i++) {
    const frag = window.tmSincopaGenerar(rng);
    const dosCompases = frag.notas.some(n => n.measure === 1);
    const c = document.createElement('div');
    c.className = 'celda' + (dosCompases ? ' ancha' : '');
    const noSincopa = frag.correctas.length === 0;
    c.innerHTML = `<span class="num">${i + 1}</span><div class="svg"></div>
      <div class="chk${solucion && noSincopa ? ' sol' : ''}">${solucion && noSincopa ? '☒' : '☐'} No hay síncopa</div>`;
    cuerpo.appendChild(c);
    window.tmSincopaDibujarImpresion(c.querySelector('.svg'), frag, solucion);
  }
  return N_FRAGMENTOS;
}
...
(async () => {
  ...
  for (const solucion of [false, true]) {
    ...
    const nombre = 'ficha-sincopa' + (solucion ? '-soluciones' : '');
