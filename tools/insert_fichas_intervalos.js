...
/* Cada pagina de intervalo lleva SUS DOS fichas: la de analizar y la de
   escribir. Antes estaban repartidas entre /terceras/ y
   /construir-intervalos/terceras/, pero esa segunda URL se fusiono aqui. */
function ficha(modo, num) {
...
function bloque(num, h2) {
  return `
${MARCA_INI}
<h2>${h2}</h2>
<p>Las mismas dos destrezas, en papel: una hoja para <strong>analizar</strong> ${PLURAL_ACC[num]} escritas y otra para <strong>escribirlas</strong>. Cada una trae <strong>${TOTAL.analizar} intervalos melódicos de ${ORD_ACC[num]}</strong> en clave de sol repartidos en siete pentagramas, empezando por notas naturales y ascendentes —las ${PLURAL_ACC[num]} ${NATURALES[num]} que salen de la escala natural— y terminando con sostenidos y bemoles y las direcciones mezcladas. Ningún ejercicio repite las mismas dos notas.</p>

<h3>Analizar ${PLURAL_ACC[num]}</h3>
<p>Debajo de cada intervalo hay una línea para escribir <strong>qué tipo de ${ORD_ACC[num]} es y si es ascendente o descendente</strong> (${num}ª M ↑, ${num}ª m ↓…).</p>
${ficha('analizar', num)}

<h3>Escribir ${PLURAL_ACC[num]}</h3>
<p>El ejercicio inverso: se da la primera nota y el intervalo que hay que formar, y el alumno coloca la segunda en el pentagrama con su alteración si la necesita.</p>
${ficha('escribir', num)}

<p>Se pueden imprimir y fotocopiar libremente para el aula. Cada hoja de soluciones es la misma ficha con la respuesta en rojo, así que se corrige superponiéndolas. El resto —incluidas las hojas que mezclan varios intervalos— está en <a href="/ejercicios/ejercicios-de-intervalos-en-pdf/">ejercicios de intervalos en PDF</a>, y si quieres hojas nuevas cada vez o practicar en pantalla eligiendo los intervalos, en el <a href="/${BASE}/a-medida/">test a medida</a>.</p>
${MARCA_FIN}
`;
}
...
function limitesBloque(html, num) {
  ...
  const pdf = url('analizar', num, false);
  if (!html.includes(pdf)) return null;
  const h2 = html.lastIndexOf('<h2>', html.indexOf(pdf));
  if (h2 < 0) return null;
  const cierre = html.indexOf('</p>', html.indexOf(url('escribir', num, true)));
...
function titulo(html, num) {
  const texto = `Fichas de ${PLURAL_ACC[num]} para imprimir (PDF)`;
...
for (const num of NUMS) {
  const destinos = [
    { file: `${BASE}/${PLURAL[num]}/index.html`, ancla: '  <p><a href="/diccionario-musical/intervalos/intervalos-musicales/">' },
  ];
  ...
  for (const d of destinos) {
    ...
    const viejo = limitesBloque(html, num);
    const sinBloque = viejo ? html.slice(0, viejo.ini) + html.slice(viejo.fin) : html;
    const nuevo = bloque(num, titulo(sinBloque, num));
