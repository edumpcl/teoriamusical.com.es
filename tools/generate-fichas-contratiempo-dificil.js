'use strict';
/**
 * Ficha imprimible de «contratiempo — nivel difícil» (A4, una cara) con su
 * hoja de soluciones. Cada fragmento tiene de 2 a 4 compases y puede tener
 * 0, 1, 2, 3 o más contratiempos a la vez (a diferencia de la ficha
 * normal, donde cada fragmento tiene como mucho uno).
 *
 *   node tools/generate-fichas-contratiempo-dificil.js            -> ficha + soluciones + preview
 *   node tools/generate-fichas-contratiempo-dificil.js --png=DIR  -> además, captura de cada hoja
 *
 * Reutiliza window.tmContratiempoGenerarDificil con un mulberry32 de
 * semilla fija, para que el PDF salga siempre igual. Verificado con
 * tools/verificar-fichas-contratiempo-dificil.js. El alumno rodea con un
 * círculo TODAS las notas que crea que están a contratiempo, o marca la
 * casilla «No hay contratiempo» si el fragmento no tiene ninguno.
 */
...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .celda { position: relative; border: 1px solid #e8e0cc; border-radius: 6px; padding: 3px 8px 4px 24px; page-break-inside: avoid; }
  .celda svg { display: block; margin: 0 auto; height: 58px !important; width: auto !important; max-width: 100% !important; }
  .num { position: absolute; top: 3px; left: 6px; font-size: 8pt; font-weight: 700; color: #9a7b28; }
  ...
  <div class="cab">
    <div class="cab-top">
      <div>
        ...
        <h1>Nota a contratiempo &mdash; nivel difícil${solucion ? '<span class="sol-tag">SOLUCIONES</span>' : ''}</h1>
      </div>
      <img class="logo" src="${LOGO}" width="34" height="35" alt="">
    </div>
    <p class="instr">Cada fragmento tiene de 2 a 4 compases. Rodea con un círculo TODAS las notas que estén a contratiempo (puede haber varias, o ninguna), o marca la casilla «No hay contratiempo» si el fragmento no tiene ninguna.</p>
    ${solucion ? '' : '<div class="datos"><span><b>Nombre:</b></span><span><b>Curso:</b></span><span><b>Fecha:</b></span></div>'}
  </div>
  <div id="cuerpo" class="lista"></div>
  <div class="pie"><span>Compases &middot; contratiempo &middot; nivel difícil${solucion ? ' &middot; soluciones' : ''} &middot; teoriamusical.com.es/ejercicios/compases/contratiempo/</span><span>teoriamusical.com.es</span></div>
</div></body></html>`;
}
...
function montar({ SEMILLA, N_FRAGMENTOS, solucion }) {
  ...
  for (let i = 0; i < N_FRAGMENTOS; i++) {
    const frag = window.tmContratiempoGenerarDificil(rng);
...
(async () => {
  ...
  for (const solucion of [false, true]) {
    ...
    const nombre = 'ficha-contratiempo-dificil' + (solucion ? '-soluciones' : '');
