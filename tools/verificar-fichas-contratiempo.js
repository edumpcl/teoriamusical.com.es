'use strict';
/* Verificador independiente de la ficha «contratiempo»: regenera los mismos
   17 fragmentos (misma semilla que generate-fichas-contratiempo.js) y
   comprueba, para cada nota, si de verdad está a contratiempo según la
   fuerza métrica real del compás. */
...
(async () => {
  ...
  const { fragmentos, compases } = await page.evaluate(({ SEMILLA, N_FRAGMENTOS }) => {
    ...
    const frags = [];
    for (let i = 0; i < N_FRAGMENTOS; i++) frags.push(window.tmContratiempoGenerar(rng));
  ...
  fragmentos.forEach(function (f, i) {
    ...
    else console.log(`#${n} OK  (${f.compasTxt})  ${f.correctas.length} contratiempo(s)  tipo=${f.tipo}`);
  });
