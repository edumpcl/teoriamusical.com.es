 ...
 *   node tools/generate-fichas-sincopa-dificil.js --png=DIR  -> además, captura de cada hoja
 *
 * Reutiliza window.tmSincopaGenerarDificil (el mismo generador del modo
 * difícil en pantalla) con un mulberry32 de semilla fija, para que el PDF
 * salga siempre igual. Verificado con tools/verificar-fichas-sincopa-dificil.js.
 * El alumno rodea con un círculo TODAS las notas donde crea que empieza una
 * síncopa, o marca la casilla «No hay síncopa» si el fragmento no tiene
 * ninguna. En soluciones, cada síncopa real sale en rojo.
 */
...
<div class="hoja">
  <div class="cab">
    <div class="cab-top">
      <div>
        ...
        <h1>Síncopa &mdash; nivel difícil${solucion ? '<span class="sol-tag">SOLUCIONES</span>' : ''}</h1>
      </div>
  ...
  <div class="pie"><span>Compases &middot; síncopa &middot; nivel difícil${solucion ? ' &middot; soluciones' : ''} &middot; teoriamusical.com.es/ejercicios/compases/sincopa/</span><span>teoriamusical.com.es</span></div>
</div></body></html>`;
}
...
function montar({ SEMILLA, N_FRAGMENTOS, solucion }) {
  ...
  for (let i = 0; i < N_FRAGMENTOS; i++) {
    const frag = window.tmSincopaGenerarDificil(rng);
...
(async () => {
  ...
  for (const solucion of [false, true]) {
    ...
    const nombre = 'ficha-sincopa-dificil' + (solucion ? '-soluciones' : '');
