'use strict';
// Un mini-pentagrama por registro del trombón (notas ESCRITAS = sonido real, clave de
// fa). Registros por octava: grave Mi2–Si2; medio Do3–Si3; agudo Do4–Si♭4.
const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');

const VEXFLOW_PATH = path.join(__dirname, 'node_modules/vexflow/build/cjs/vexflow.js');
const OUTPUT_DIR = path.join(__dirname, 'assets/img/trombon');
fs.mkdirSync(OUTPUT_DIR, { recursive: true });

const ITEMS = [
  ['tesitura-grave', ['e/2', 'b/2'],  100],
  ['tesitura-medio', ['c/3', 'b/3'],  105],
  ['tesitura-agudo', ['c/4', 'bb/4'], 110],
];
...
function renderReg(def) {
  ...
  const notes = def.keys.map(function (k) {
    const n = new StaveNote({ keys: [k], duration: 'w', clef: 'bass' });
