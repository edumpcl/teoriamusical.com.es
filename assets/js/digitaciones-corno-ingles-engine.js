...
(function () {
  ...
  /* OJO: estas son las fotos del OBOE, reutilizadas a proposito.
     El corno ingles es un oboe en Fa: misma mecanica de conservatorio y su lista de
     llaves es la del oboe MENOS el Si bemol grave del menique izquierdo (LP4), que aqui
     no existe. No hay ninguna foto util de corno ingles: la de Commons esta en diagonal
     y deja el cuerpo en unos 133 px de ancho, cuando el oboe necesito 495. Se avisa en
     el texto de la pagina y bajo el diagrama.
  ...
  var FOTOS = {
    sup: {
      ...
      k: {
        ...
        LP1: [395, 1059, 18], LP2:[341, 1155, 18], LP3:[423, 1133, 18]
      }
    },
    inf: {
  ...
  var CSS = [
    ...
    '.tm-ci-img{display:block;width:min(30vw,178px);height:auto;border-radius:6px;object-fit:contain;}',
    '.tm-ci-svg{position:absolute;inset:0;width:100%;height:100%;overflow:visible;}',
    '.tm-ci-cap{font-size:.75rem;color:#888;text-align:center;margin:0;line-height:1.3;}',
    // marcador sobre la foto
    '.tm-ci-key .k-dot{fill:#ff9500;fill-opacity:0;stroke:rgba(255,255,255,0);stroke-width:0;transition:all .16s;}',
    '.tm-ci-key.on .k-dot{fill:#ff9500;fill-opacity:.92;stroke:#fff;stroke-width:5;filter:drop-shadow(0 0 14px #ff9500);}',
    // medio agujero: media luna sobre el agujero 1
    '.tm-ci-key .k-half{fill:#ff9500;fill-opacity:0;stroke:rgba(255,255,255,0);stroke-width:0;transition:all .16s;}',
    '.tm-ci-key.on .k-half{fill:#ff9500;fill-opacity:.92;stroke:#fff;stroke-width:5;filter:drop-shadow(0 0 14px #ff9500);}',
    ...
    '.tm-ci-thlab{font-family:Arial,Helvetica,sans-serif;font-size:12px;fill:#666;text-anchor:middle;}',
    '.tm-ci-credit{font-size:.72rem;color:#9a9a9a;text-align:center;margin-top:8px;}',
    '.tm-ci-credit a{color:inherit;}',
    ...
    '@media(max-width:600px){.tm-ci-cols{gap:8px;}.tm-ci-col{flex:0 1 48%;min-width:0;}.tm-ci-img{width:100%;}.tm-ci-cap{font-size:.62rem;}}'
  ].join('');
  ...
  function columna(cfg) {
    ...
    // y descoloca los marcadores. Lo que sobresale no se recorta porque .tm-ci-svg lleva
    // overflow:visible.
    var vb = '0 0 ' + cfg.w + ' ' + cfg.h;
    return '<div class="tm-ci-col">' +
      '<p class="tm-ci-cap">' + cfg.titulo + '</p>' +
      '<div class="tm-ci-foto">' +
        '<picture><source type="image/webp" srcset="/assets/img/oboe/' + cfg.img + '.webp">' +
        '<img class="tm-ci-img" src="/assets/img/oboe/' + cfg.img + '.jpg" width="' + cfg.w + '" height="' + cfg.h + '" loading="lazy" alt="' + cfg.alt + '"></picture>' +
        '<svg class="tm-ci-svg" viewBox="' + vb + '" preserveAspectRatio="xMidYMid meet" role="img" aria-label="' + cfg.alt + '">' + marcadores(cfg) + '</svg>' +
      '</div>' +
    '</div>';
  }
  ...
  function buildPulgar() {
    return '<svg class="tm-ci-pulgarsvg" viewBox="0 0 200 120" role="img" aria-label="Cara oculta del oboe: la primera llave de octava, que acciona el pulgar izquierdo, se ilumina cuando hay que pulsarla">' +
      '<defs>' +
        '<linearGradient id="tmCiMet" x1="0" y1="0" x2="1" y2="0">' +
          '<stop offset="0" stop-color="#c9ccd3"/><stop offset=".45" stop-color="#f2f3f6"/><stop offset="1" stop-color="#9fa3ab"/>' +
        '</linearGradient>' +
        '<linearGradient id="tmCiMetOn" x1="0" y1="0" x2="1" y2="0">' +
      ...
      '<g class="tm-ci-key" data-k="OCT1">' +
        '<rect class="tm-ci-th-pad" x="66" y="46" width="68" height="22" rx="11"/>' +
      ...
      '<text class="tm-ci-thlab" x="100" y="118">pulgar izquierdo</text>' +
      '</svg>';
  }
  ...
  function tmCornoInglesEngine(containerId) {
    ...
    wrap.innerHTML =
      '<div class="tm-ci-wrap">' +
        ...
        '<div class="tm-ci-diagram">' +
          '<div class="tm-ci-cols">' +
            ORDEN_FOTOS.map(function (n) { return columna(FOTOS[n]); }).join('') +
          '</div>' +
          '<div class="tm-ci-pulgar">' +
            buildPulgar() +
            '<p class="tm-ci-pulgarcap"><strong>Por detr\u00e1s.</strong> La <strong>1.\u00aa llave de octava</strong> la acciona el <strong>pulgar izquierdo</strong> en la cara oculta del tubo, as\u00ed que no sale en las fotos y se dibuja aparte.</p>' +
          '</div>' +
          '<p class="tm-ci-credit"><strong>El corno ingl\u00e9s se digita igual que el oboe</strong> y lleva la misma mec\u00e1nica, as\u00ed que el diagrama muestra un <strong>oboe</strong>. Fotos: oboe Yamaha YOB-831, Yamaha Corporation v\u00eda <a href="https://commons.wikimedia.org/wiki/File:Yamaha_Oboe_YOB-831.tif" target="_blank" rel="noopener">Wikimedia Commons</a>, CC BY-SA 4.0. Llave de octava del pulgar: diagrama propio.</p>' +
        '</div>' +
    ...
    function pick(n, btn) {
      ...
      var data = FING[n];
      wrap.querySelectorAll('.tm-ci-key').forEach(function (c) { c.classList.remove('on'); });
      if (data) data.keys.forEach(function (id) {
        wrap.querySelectorAll('.tm-ci-key[data-k="' + id + '"]').forEach(function (c) { c.classList.add('on'); });
      });
