/* Diagrama de digitaciones del clarinete bajo (sistema Boehm) — interactivo.
   El clarinete bajo hereda las digitaciones del clarinete en Si♭ (misma carta
   Yamaha verificada en digitaciones-clarinete-engine.js) con dos diferencias:
   añade el Mi♭3 grave escrito (llave propia junto al meñique derecho) y, aunque
   también está en Si♭, suena una NOVENA MAYOR (octava + tono) por debajo de lo
   escrito. Visual: cuello curvado metálico con la boquilla y campana metálica
   vuelta hacia arriba. El marcador muestra ambas alturas y el audio reproduce
   la altura real. Uso: <div id="x"></div><script>tmClarineteBajoEngine('x');</script> */
(function () {
  'use strict';
  ...
  var KEYS = [
    ...
    { id: 'EBG',  sh: 'loop',  x: 500, y: 222 }   // Mi♭ grave (solo clarinete bajo)
  ];
  ...
  // El clarinete bajo en Si♭ suena una novena mayor (14 semitonos) por debajo de lo escrito
  function suena(n) {
  ...
  var CSS = [
    ...
    '.tm-cb-diagram{background:#fff;border:1px solid #e8e0cc;border-radius:8px;padding:6px;display:flex;justify-content:center;}',
    '.tm-cb-svg{display:block;max-width:640px;width:100%;height:auto;margin:0 auto;}',
    '.tm-cb-key .k-pad{fill:#e9eaee;stroke:#8f9199;stroke-width:1.5;}',
    '.tm-cb-key.on .k-pad{fill:#8b6914;stroke:#6b5010;}',
    '.tm-cb-key .k-ring{fill:none;stroke:#8f9199;stroke-width:1.2;}',
    '.tm-cb-key.on .k-ring{stroke:#e8dcc0;}',
    '.tm-cb-klab{font-family:Arial,Helvetica,sans-serif;font-size:11px;fill:#555;text-anchor:middle;}',
    '.tm-cb-klab2{font-family:Arial,Helvetica,sans-serif;font-size:9px;fill:#777;text-anchor:middle;}',
    '.tm-cb-grp{font-family:Arial,Helvetica,sans-serif;font-size:12px;fill:#8b6914;text-anchor:middle;font-weight:bold;}',
  ...
  var DECO =
    '<defs><linearGradient id="tmCbWood" x1="0" y1="0" x2="0" y2="1">' +
    ...
    '</linearGradient></defs>' +
    // boquilla sobre CUELLO CURVADO metálico (característico del clarinete bajo)
    ...
    '<path d="M122 151 L560 148 L560 185.5 L122 181 Z" fill="url(#tmCbWood)" stroke="#0d0b0a" stroke-width="1"/>' +
    ...
    '<ellipse cx="652" cy="78" rx="10" ry="22" fill="url(#tmFnMetalCB)" stroke="#8f9199" stroke-width="1.2" transform="rotate(28 652 78)"/>' +
    '<defs><linearGradient id="tmFnMetalCB" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#f2f3f5"/><stop offset="1" stop-color="#a9abb2"/></linearGradient></defs>' +
    ...
    // etiquetas
    '<text class="tm-cb-klab" x="44" y="76">Boquilla</text>' +
    '<text class="tm-cb-klab" x="86" y="134">Cuello curvado</text>' +
    '<text class="tm-cb-grp" x="152" y="88">Pulgar (detrás)</text>' +
    '<text class="tm-cb-klab2" x="139" y="100">registro · pulgar</text>' +
    '<text class="tm-cb-grp" x="216" y="102">Garganta</text>' +
    '<text class="tm-cb-klab" x="595" y="200">Campana metálica</text>' +
    '<text class="tm-cb-klab" x="215" y="205">1</text><text class="tm-cb-klab" x="250" y="205">2</text><text class="tm-cb-klab" x="285" y="205">3</text>' +
    '<text class="tm-cb-klab" x="345" y="206">4</text><text class="tm-cb-klab" x="380" y="206">5</text><text class="tm-cb-klab" x="415" y="206">6</text>' +
    '<text class="tm-cb-klab2" x="268" y="143">banana</text>' +
    '<text class="tm-cb-klab2" x="363" y="144">desliz.</text>' +
    '<text class="tm-cb-klab" x="308" y="84">Meñique izq.</text>' +
    '<text class="tm-cb-klab" x="454" y="252">Meñique dcho.</text>' +
    '<text class="tm-cb-grp" x="250" y="274">Mano izquierda</text>' +
    '<text class="tm-cb-grp" x="415" y="274">Mano derecha</text>';

  function keyShape(k) {
    var open = '<g class="tm-cb-key" data-k="' + k.id + '">', close = '</g>';
    if (k.sh === 'oct') {
  ...
  function tmClarineteBajoEngine(containerId) {
    ...
    wrap.innerHTML =
      '<div class="tm-cb-wrap">' +
        ...
        '<div class="tm-cb-diagram"><svg class="tm-cb-svg" viewBox="0 0 ' + SVG_W + ' ' + SVG_H + '" role="img" aria-label="Diagrama de digitación del clarinete bajo: instrumento horizontal con el cuello curvado y la boquilla a la izquierda y la campana metálica vuelta hacia arriba a la derecha; las llaves pulsadas se muestran en dorado">' +
          DECO + keysSvg +
        '</svg></div>' +
    ...
    function pick(n, btn) {
      ...
      var data = FING[n];
      svg.querySelectorAll('.tm-cb-key').forEach(function (c) { c.classList.remove('on'); });
      var nombres = [];
      if (data) data.keys.forEach(function (id) {
        var c = svg.querySelector('.tm-cb-key[data-k="' + id + '"]'); if (c) c.classList.add('on');
      ...
      ro.innerHTML =
        ...
        '<div class="tm-cb-reg">' + intl(n) + ' · ' + registro(n) + ' · suena ' + suena(n) + ' (9ª abajo)</div>' +
