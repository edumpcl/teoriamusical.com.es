/* Diagrama de digitaciones del clarinete contrabajo (sistema Boehm) — interactivo.
   El clarinete contrabajo hereda las digitaciones del clarinete en Si♭ (misma
   carta Yamaha), incluida la llave del Mi♭3 grave escrito. Está en Si♭ y suena
   una 16ª MAYOR —dos octavas y un tono, 26 semitonos— por debajo de lo escrito
   (una octava más grave que el clarinete contrabajo). Diagrama VERTICAL, como se toca.
   El marcador muestra ambas alturas y el audio reproduce la altura real. Uso:
   <div id="x"></div><script>tmClarineteContrabajoEngine('x');</script> */
(function () {
  'use strict';
  ...
  var KEYS = [
    ...
    { id: 'EBG',  sh: 'loop',  x: 500, y: 222 }   // Mi♭ grave (escrita; el contrabajo también la lleva)
  ];
  ...
  var CSS = [
    ...
    '.tm-cc-diagram{background:#fff;border:1px solid #e8e0cc;border-radius:8px;padding:6px;display:flex;justify-content:center;}',
    '.tm-cc-svg{display:block;height:min(72vh,560px);width:auto;max-width:100%;margin:0 auto;}',
    '.tm-cc-key .k-pad{fill:#e9eaee;stroke:#8f9199;stroke-width:1.5;}',
    '.tm-cc-key.on .k-pad{fill:#8b6914;stroke:#6b5010;}',
    '.tm-cc-key .k-ring{fill:none;stroke:#8f9199;stroke-width:1.2;}',
    '.tm-cc-key.on .k-ring{stroke:#e8dcc0;}',
    '.tm-cc-klab{font-family:Arial,Helvetica,sans-serif;font-size:11px;fill:#555;text-anchor:middle;}',
    '.tm-cc-klab2{font-family:Arial,Helvetica,sans-serif;font-size:9px;fill:#777;text-anchor:middle;}',
    '.tm-cc-grp{font-family:Arial,Helvetica,sans-serif;font-size:12px;fill:#8b6914;text-anchor:middle;font-weight:bold;}',
  ...
  var DECO =
    '<defs><linearGradient id="tmCcWood" x1="0" y1="0" x2="0" y2="1">' +
    ...
    '</linearGradient></defs>' +
    // boquilla sobre CUELLO CURVADO metálico (característico del clarinete contrabajo)
    ...
    '<path d="M122 151 L560 148 L560 185.5 L122 181 Z" fill="url(#tmCcWood)" stroke="#0d0b0a" stroke-width="1"/>' +
    ...
    '<ellipse cx="652" cy="78" rx="10" ry="22" fill="url(#tmFnMetalCC)" stroke="#8f9199" stroke-width="1.2" transform="rotate(28 652 78)"/>' +
    '<defs><linearGradient id="tmFnMetalCC" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#f2f3f5"/><stop offset="1" stop-color="#a9abb2"/></linearGradient></defs>' +
    ...
    // etiquetas
    '<text class="tm-cc-klab" x="44" y="76">Boquilla</text>' +
    '<text class="tm-cc-klab" x="86" y="134">Cuello curvado</text>' +
    '<text class="tm-cc-grp" x="152" y="88">Pulgar (detrás)</text>' +
    '<text class="tm-cc-klab2" x="139" y="100">registro · pulgar</text>' +
    '<text class="tm-cc-grp" x="216" y="102">Garganta</text>' +
    '<text class="tm-cc-klab" x="595" y="200">Campana metálica</text>' +
    '<text class="tm-cc-klab" x="215" y="205">1</text><text class="tm-cc-klab" x="250" y="205">2</text><text class="tm-cc-klab" x="285" y="205">3</text>' +
    '<text class="tm-cc-klab" x="345" y="206">4</text><text class="tm-cc-klab" x="380" y="206">5</text><text class="tm-cc-klab" x="415" y="206">6</text>' +
    '<text class="tm-cc-klab2" x="268" y="143">banana</text>' +
    '<text class="tm-cc-klab2" x="363" y="144">desliz.</text>' +
    '<text class="tm-cc-klab" x="308" y="84">Meñique izq.</text>' +
    '<text class="tm-cc-klab" x="454" y="252">Meñique dcho.</text>' +
    '<text class="tm-cc-grp" x="250" y="274">Mano izquierda</text>' +
    '<text class="tm-cc-grp" x="415" y="274">Mano derecha</text>';

  function keyShape(k) {
    var open = '<g class="tm-cc-key" data-k="' + k.id + '">', close = '</g>';
    if (k.sh === 'oct') {
  ...
  function tmClarineteContrabajoEngine(containerId) {
    ...
    wrap.innerHTML =
      '<div class="tm-cc-wrap">' +
        ...
        '<div class="tm-cc-diagram"><svg class="tm-cc-svg" viewBox="0 0 300 ' + SVG_W + '" role="img" aria-label="Diagrama de digitación del clarinete contrabajo: instrumento en vertical (como se toca), con la boquilla y el cuello curvado arriba y la campana metálica abajo; las llaves pulsadas se muestran en dorado">' +
          '<g transform="rotate(90) translate(0,-300)">' + DECOV + keysSvg + '</g>' +
        '</svg></div>' +
        '<div class="tm-cc-btns">' + btns + '</div>' +
      '</div>';

    var svg = wrap.querySelector('.tm-cc-svg');
    ...
    function pick(n, btn) {
      ...
      var data = FING[n];
      svg.querySelectorAll('.tm-cc-key').forEach(function (c) { c.classList.remove('on'); });
      var nombres = [];
      if (data) data.keys.forEach(function (id) {
        var c = svg.querySelector('.tm-cc-key[data-k="' + id + '"]'); if (c) c.classList.add('on');
      ...
      ro.innerHTML =
        ...
        '<div class="tm-cc-reg">' + intl(n) + ' · ' + registro(n) + ' · suena ' + suena(n) + ' (16ª abajo)</div>' +
