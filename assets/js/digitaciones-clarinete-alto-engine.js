/* Diagrama de digitaciones del clarinete alto (en Mi♭, sistema Boehm) — interactivo.
   ...
   Transposición: suena una 6ª mayor (9 semitonos) por DEBAJO de lo escrito.
   Uso: <div id="x"></div><script>tmClarineteAltoEngine('x');</script> */
(function () {
  'use strict';

  // ── FRENTE: foto real del alto (Yamaha YCL-631 II), marcadores calibrados (viewBox 562×880) ──
  var FRONT_W = 562, FRONT_H = 880;
  var FRONT_KEYS = [
    { id: 'GS',   x: 373, y: 267, r: 13 },
    { id: 'A',    x: 322, y: 239, r: 15 },
    { id: 'H1',   x: 319, y: 290, r: 22, num: 1 },
    { id: 'H2',   x: 323, y: 389, r: 22, num: 2 },
    { id: 'EBBA', x: 324, y: 417, r: 13 },
    { id: 'H3',   x: 323, y: 451, r: 22, num: 3 },
    { id: 'H4',   x: 328, y: 651, r: 22, num: 4 },
    { id: 'SLI',  x: 325, y: 763, r: 13 },
    { id: 'H5',   x: 325, y: 732, r: 22, num: 5 },
    { id: 'H6',   x: 329, y: 796, r: 22, num: 6 }
  ];

  // ── MEÑIQUES (dibujo). El alto añade EBG (Mi♭ grave) como 5ª espátula del racimo dcho. ──
  ...
  var PK_DER = [
    ...
    { id: 'EBG',  cx: 96,  cy: 128, w: 60, h: 22, a: 2 }   // Mi♭ grave (extensión del alto)
  ];
  function pkGrad() {
    return '<defs><linearGradient id="tmcaPkMet" x1="0" y1="0" x2="0.4" y2="1">' +
      ...
      '<linearGradient id="tmcaPkMetOn" x1="0" y1="0" x2="0.4" y2="1">' +
  ...
  function pkSpat(s) {
    ...
    return '<g class="tm-ca-key" data-k="' + s.id + '" transform="translate(' + s.cx + ' ' + s.cy + ') rotate(' + s.a + ')">' +
      '<rect class="tm-ca-pk-pad" x="' + (-hw) + '" y="' + (-hh) + '" width="' + s.w + '" height="' + s.h + '" rx="' + hh + '"/>' +
      '<rect class="tm-ca-pk-hl" x="' + (-hw + 5) + '" y="' + (-hh + 3) + '" width="' + (s.w - 10) + '" height="' + (s.h * 0.28).toFixed(1) + '" rx="' + (s.h * 0.14).toFixed(1) + '"/>' +
      '</g>';
  }
  function buildPinkySvg(list, vb, label) {
    return '<div class="tm-ca-pkcluster">' +
      '<svg class="tm-ca-pksvg" viewBox="' + vb + '" role="img" aria-label="Espátulas del ' + label + '; la que se pulsa se ilumina en dorado">' +
      pkGrad() + list.map(pkSpat).join('') + '</svg>' +
      '<p class="tm-ca-pkcap">' + label + '</p></div>';
  }
  function buildBack() {
    return '<svg class="tm-ca-backsvg" viewBox="46 104 112 158" role="img" aria-label="Parte de atrás del clarinete alto (diagrama): la llave de registro (arriba) y el agujero del pulgar (abajo) se iluminan al usarse">' +
      pkGrad() +
      '<g class="tm-ca-key" data-k="R">' +
        '<rect class="tm-ca-pk-pad" x="70" y="118" width="62" height="20" rx="10" transform="rotate(-6 101 128)"/>' +
        '<circle class="tm-ca-pk-pad" cx="84" cy="150" r="13"/>' +
      '</g>' +
      '<g class="tm-ca-key" data-k="T"><circle class="tm-ca-holeD" cx="91" cy="222" r="21"/></g>' +
      '</svg>';
  }
  function keyShape(k) {
    var s = '<g class="tm-ca-key" data-k="' + k.id + '"><circle class="k-dot" cx="' + k.x + '" cy="' + k.y + '" r="' + k.r + '"/>';
  ...
  // El clarinete alto en Mi♭ suena una 6ª mayor (9 semitonos) por DEBAJO de lo escrito
  function suena(n) {
    var midi = midiOf(n) - 9;
  ...
  var CSS = [
    ...
    '.tm-ca-diagram{background:#fff;border:1px solid #e8e0cc;border-radius:8px;padding:10px 8px;}',
    '.tm-ca-photos{display:flex;gap:16px;justify-content:center;align-items:flex-start;flex-wrap:wrap;}',
    '.tm-ca-photo{position:relative;flex:0 0 auto;}',
    '.tm-ca-front img{display:block;height:min(50vh,440px);width:auto;border-radius:6px;}',
    '.tm-ca-back{display:flex;flex-direction:column;align-items:center;gap:6px;flex:0 0 auto;align-self:center;}',
    '.tm-ca-img{display:block;border-radius:6px;}',
    '.tm-ca-svg{position:absolute;inset:0;width:100%;height:100%;overflow:visible;}',
    '.tm-ca-backcap{font-size:.8rem;color:#777;max-width:180px;text-align:center;margin:0;line-height:1.3;}',
    '.tm-ca-backcap strong{color:#555;}',
    '.tm-ca-pinky{display:flex;flex-direction:column;align-items:center;flex:0 0 auto;align-self:center;gap:10px;}',
    '.tm-ca-pkcluster{display:flex;flex-direction:column;align-items:center;gap:2px;}',
    '.tm-ca-pksvg{width:min(38vw,170px);height:auto;}',
    '.tm-ca-backsvg{width:min(22vw,100px);height:auto;}',
    '.tm-ca-pk-pad{fill:url(#tmcaPkMet);stroke:#7f828a;stroke-width:1.5;transition:fill .15s,stroke .15s;}',
    '.tm-ca-key.on .tm-ca-pk-pad{fill:url(#tmcaPkMetOn);stroke:#b06a00;}',
    '.tm-ca-pk-hl{fill:#fff;opacity:.5;pointer-events:none;}',
    '.tm-ca-holeD{fill:#241f1b;stroke:#0c0a08;stroke-width:2;transition:fill .15s,stroke .15s;}',
    '.tm-ca-key.on .tm-ca-holeD{fill:#ff9500;stroke:#fff;}',
    '.tm-ca-pkcap{font-size:.8rem;color:#777;max-width:230px;text-align:center;margin:2px 0 0;line-height:1.3;}',
    '.tm-ca-key .k-dot{fill:#ff9500;fill-opacity:0;stroke:rgba(255,255,255,0);stroke-width:0;transition:all .16s;}',
    '.tm-ca-key.on .k-dot{fill:#ff9500;fill-opacity:.92;stroke:#fff;stroke-width:2.4;filter:drop-shadow(0 0 6px #ff9500);}',
    '.tm-ca-key .k-num{font-family:Arial,Helvetica,sans-serif;font-weight:bold;fill:#fff;fill-opacity:.55;text-anchor:middle;dominant-baseline:central;paint-order:stroke;stroke:#000;stroke-width:.6px;stroke-opacity:.5;transition:all .16s;pointer-events:none;}',
    '.tm-ca-key.on .k-num{fill:#3a2b00;fill-opacity:1;stroke-opacity:0;}',
    '.tm-ca-credit{font-size:.72rem;color:#9a9a9a;text-align:center;margin-top:8px;}',
    '.tm-ca-credit a{color:inherit;}',
  ...
  function tmClarineteAltoEngine(containerId) {
    ...
    wrap.innerHTML =
      '<div class="tm-ca-wrap">' +
        ...
        '<div class="tm-ca-diagram">' +
          '<div class="tm-ca-photos">' +
            '<div class="tm-ca-photo tm-ca-front">' +
              '<picture><source type="image/webp" srcset="/assets/img/clarinete-alto/digitacion-frente.webp">' +
              '<img class="tm-ca-img" src="/assets/img/clarinete-alto/digitacion-frente.jpg" width="' + FRONT_W + '" height="' + FRONT_H + '" loading="lazy" alt="Clarinete alto en Mi♭ visto de frente; los agujeros y llaves que se pulsan se iluminan en dorado"></picture>' +
              '<svg class="tm-ca-svg" viewBox="0 0 ' + FRONT_W + ' ' + FRONT_H + '" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Digitación del clarinete alto sobre una fotografía real (cara frontal)">' + frontSvg + '</svg>' +
            '</div>' +
            '<div class="tm-ca-back">' +
              buildBack() +
              '<p class="tm-ca-backcap"><strong>Parte de atrás</strong><br>el pulgar izquierdo tapa su agujero y aprieta la <strong>llave de registro</strong> (la 12ª).</p>' +
            '</div>' +
            '<div class="tm-ca-pinky">' +
              pinkyIzq + pinkyDer +
              '<p class="tm-ca-pkcap">Los <strong>meñiques</strong>: racimos laterales que no se ven de frente.</p>' +
            '</div>' +
          '</div>' +
          '<p class="tm-ca-credit">Foto del frente: clarinete alto Yamaha YCL-631 II, Yamaha Corporation vía <a href="https://commons.wikimedia.org/wiki/File:Yamaha_Alto_Clarinet_YCL-631_II.jpg" target="_blank" rel="noopener">Wikimedia Commons</a>, CC BY-SA 4.0. Dorso y meñiques: diagrama propio.</p>' +
        '</div>' +
    ...
    function pick(n, btn) {
      ...
      var data = FING[n];
      wrap.querySelectorAll('.tm-ca-key').forEach(function (c) { c.classList.remove('on'); });
      var nombres = [];
      if (data) data.keys.forEach(function (id) {
        wrap.querySelectorAll('.tm-ca-key[data-k="' + id + '"]').forEach(function (c) { c.classList.add('on'); });
